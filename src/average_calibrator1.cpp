...
class AverageCalibrator
{
    private:
        ...
        ros::ServiceServer srv_calibrate_full_;
    ...
    public:
        ...
        bool calibrate_final(std_srvs::Empty::Request &request, std_srvs::Empty::Response &response);
        ...
        AverageCalibrator(ros::NodeHandle nh) : nh_(nh), priv_nh_("~")
        {
            ...
            srv_calibrate_full_ = nh_.advertiseService(nh_.resolveName("calibrate_full"), &AverageCalibrator::calibrate_final, this);
...
bool AverageCalibrator::calibrate_final( std_srvs::Empty::Request &request, std_srvs::Empty::Response &response )
{
    int n = transform_vector_.size();
    double tx(0),ty(0),tz(0),ax(0),ay(0),az(0),angle(0);

    for(int i = 0; i<n; i++)
    {
    Eigen::AngleAxisd first_ax ( Eigen::Quaterniond(transform_vector_.at(i).getRotation().getW(), transform_vector_.at(i).getRotation().getX(), 
